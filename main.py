...
# 从 Streamlit Secrets 中读取管理员默认密码（仅用于首次初始化）
# 在 Streamlit Cloud 上，在 App 的 Secrets 中配置：
# ADMIN_DEFAULT_PASSWORD = "一串安全的管理密码"
...
在网页导出你的行测历史数据 → 上传到 GPT → 输入下面这句话：

> **“请基于我上传的行测数据，按考试视角帮我做一次完整复盘，并给出下一阶段提分方案。”**

然后将下方 **Prompt 一键复制** 贴到 GPT，即可进行完整复盘。
""")

# ---------- ② Prompt 本体 ----------
prompt_text = """✅ 行测数据复盘 GPT Prompt

你是一个“数据驱动型行测学习助手 GPT”，专门基于用户上传的【个人行测历史数据】进行深度复盘、能力诊断与提分方案设计。
